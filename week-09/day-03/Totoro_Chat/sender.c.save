...
void connect_to_server(SOCKET *client_sock, char *server_ip, int server_port)
{
	...
	// Connecting the c lient socket to the server
...
void send_message_to_discovery(char *partner_ip, int partner_discovery_port)
{
	// Connect to server
	SOCKET client_socket;
	connect_to_server(&client_socket, partner_ip, partner_discovery_port);

	int sent_bytes;
		// Send data to the server
    sent_bytes = send_message(&client_socket, "Marci 8484");
